...
`POST /clocks/:id/adjustments` 的字段要求：

- `currentDailyRateSeconds`（走时误差）必须是有限数字（可传数字字符串，如 `"68.5"`），
  布尔值、无法转数字的文本、`null`、缺失均拒绝；
- `direction`（方向）与 `amount`（操作量）必须是去空白后非空的字符串，
  `null`、缺失、纯空白串、数字、布尔、对象、数组均拒绝。

三类字段的任何非法输入都在写库前拦截，不会产生调校记录。
